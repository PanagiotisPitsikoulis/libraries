...
async function runLocalSQL(command: string, database = 'postgres'): Promise<string> {
  try {
    const result =
...
async function main() {
  ...
  try {
    ...
    logInfo('📤 Creating backup copy from cloud database (read-only operation)...')
    try {
    ...
    logInfo('🗑️  Dropping existing local database (cloud database remains untouched)...')
    ...
    console.log(`postgresql://${LOCAL_DB_USER}@${LOCAL_DB_HOST}/${LOCAL_DB_NAME}`)
  } catch (error) {
