...
config({ path: join(__dirname, 'db.conf') })
...
async function main() {
  ...
  try {
    ...
    await runSQL(`ALTER ROLE ${APP_USER} SET idle_session_timeout = '${DB_IDLE_SESSION_TIMEOUT}'`)
    await runSQL(`ALTER SYSTEM SET idle_session_timeout = '${DB_MASTER_IDLE_TIMEOUT}'`)
    await runSQL(`ALTER SYSTEM SET tcp_keepalives_idle = ${DB_TCP_KEEPALIVES_IDLE}`)
    await runSQL(`ALTER SYSTEM SET tcp_keepalives_interval = ${DB_TCP_KEEPALIVES_INTERVAL}`)
    await runSQL(`ALTER SYSTEM SET tcp_keepalives_count = ${DB_TCP_KEEPALIVES_COUNT}`)
