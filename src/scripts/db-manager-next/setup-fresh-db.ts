...
import { executeStep, logError, logInfo, logSuccess, logWarning } from '../utils'
...
async function runSQL(command: string, database = 'postgres'): Promise<string> {
  try {
    ...
    logError(`SQL command failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
    return ''
  }
}
...
async function main() {
  ...
  const dbExists = await runSQL(`SELECT 1 FROM pg_database WHERE datname='${CLOUD_DB_NAME}'`)
  ...
  await runSQL(`ALTER DATABASE ${CLOUD_DB_NAME} CONNECTION LIMIT ${DB_MAX_CONNECTIONS};`)
  ...
  await executeStep('Setting timeouts', 'bun', ['run', join(__dirname, 'set-timeouts.ts')])
